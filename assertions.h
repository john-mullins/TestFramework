...
namespace UnitTests
{
	...
	public:
        ...
        void In(std::string needle, std::string haystack) const
		{
			...
			if (!present)
			{
				Error(img::format("Expected to find \"%s\" in string \"%s\" ", img::escape_string_literal(needle), img::escape_string_literal(haystack)));
			}
		}
        ...
        void NotIn(std::string needle, std::string haystack) const
		{
			...
			if (present)
			{
				Error(img::format("Did not expect to find \"%s\" in string \"%s\" ", img::escape_string_literal(needle), img::escape_string_literal(haystack)));
			}
		}
