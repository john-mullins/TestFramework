...
namespace UnitTests
{
    ...
    namespace details
    {
        ...
        struct expected_got_outputter_with_coercion
        {
            expected_got_outputter_with_coercion(const T& first_type, const U& second_type)
                : t(first_type), u(second_type)
            {
            }
            const T& t;
            const U& u;
        ...
        template <class T, class U>
        std::ostream& streamit(
            std::ostream& s, const expected_got_outputter_with_coercion<T, U>& t, std::true_type /*unused*/)
        {
            return s << stream(t.t);
        }
