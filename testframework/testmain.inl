...
namespace UnitTests
{
    ...
    public:
        ...
        int report() override
        {
            ...
            return count_tests(Failed) + count_tests(Error);
            ;
        }
    ...
    {
        return std::any_of(begin(args), end(args), [](const std::string& s) { return s == "-v" || s == "--verbose"; });
    }
    ...
    int MiniSuite::RunTests(const std::vector<std::string>& args, std::ostream& os)
    {
        ...
        auto reporter = xml.empty() ? std::unique_ptr<Reporter>(new StreamReporter(os, verbose)) :
                                      std::unique_ptr<Reporter>(new XMLReporter(xml));
